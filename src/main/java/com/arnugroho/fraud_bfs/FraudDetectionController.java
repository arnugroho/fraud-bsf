...
public class FraudDetectionController {
    ...
    @GetMapping("/check")
    public ResponseEntity<List<String>> runCheckFraud() throws IOException {
        List<String> frauds = checkFraud(getJson());

        return ResponseEntity.status(200).body(frauds.isEmpty() ? List.of("Tidak ada fraud terdeteksi.") : frauds);
    }
    ...
    public ResponseEntity<List<String>> uploadFile(@RequestParam("file") MultipartFile file) {
        try {
            ...
            // Convert the uploaded file to a list of Transaction objects
            List<Transaksi> transactions = objectMapper.readValue(file.getInputStream(), new TypeReference<List<Transaksi>>() {});


            List<String> frauds = checkFraud(transactions);
    ...
    private List<String> checkFraud(List<Transaksi> dataList) throws IOException {
